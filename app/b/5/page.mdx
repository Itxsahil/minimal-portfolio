export const metadata = {
  ...
  openGraph: {
    images: [
      {
        url: "/api/og?title=Understanding%20WebRTC%20%E2%80%94%20Real-Time%20Communication%20in%20the%20Browser",
        width: 1200,
        height: 630,
      },
    ],
...
- By **low latency**, we mean a protocol designed to minimize delay, and that’s why WebRTC is built on top of **UDP**.
...
The problem:
If someone outside wants to connect to _you_, they don’t know your private IP. They only see your router’s public IP. Unless your router explicitly forwards the traffic to your device (port forwarding), the connection will fail.
...
- Your client asks a STUN server: “Hey, what’s my public IP and port?”
...
The exchange of SDP messages is done via **signaling** (remember: WebRTC doesn’t dictate how).
...
At this point, you’ll see a JSON object logged in the console that looks like:

```json
{ "type": "offer", "sdp": "v=0..." }
```

Copy that JSON (the whole object) — you’ll need it for Client 2.
...
Now you’ll see another JSON object in Client 2’s console that looks like:
...
At this point, both clients should log **“Data channel open!”**
...
Each side will see the messages appear in its console. 🎉
...
- NAT traversal is complex, doesn’t always work perfectly
